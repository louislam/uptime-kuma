...
class UptimeKumaServer {
    ...
    constructor() {
        ...
        // Set Monitor Types
        loadPlugins(path.join(__dirname, "monitor-types"), MonitorType, (instance, filename) => {
            if (!instance.name) {
                throw new Error(`Monitor type in file "${filename}" missing 'name' property`);
            }

            if (UptimeKumaServer.monitorTypeList[instance.name]) {
                throw new Error(`Duplicate monitor type "${instance.name}" in file "${filename}"`);
            }
            UptimeKumaServer.monitorTypeList[instance.name] = instance;
        });
