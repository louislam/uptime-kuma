...
class UptimeKumaServer {
    ...
    async getMonitorJSONList(userID, monitorID = null) {
        let query = `
            ...
            WHERE (monitor.user_id = ? OR monitor_access.user_id IS NOT NULL)
        `;
        const params = [ userID, userID ];

        if (monitorID) {
            query += " AND monitor.id = ?";
            params.push(monitorID);
        }

        query += " ORDER BY monitor.weight DESC, monitor.name";

        const rows = await R.getAll(query, params);
        const monitorList = [];
        for (const row of rows) {
            const bean = await R.findOne("monitor", " id = ? ", [ row.id ]);
            if (bean) {
                monitorList.push(bean);
            }
        }
        ...
        for (let monitor of monitorList) {
            // Attach permission info for the client to decide what actions to show
            const permissionRow = await R.getRow(`
                SELECT CASE WHEN monitor.user_id = ? THEN 'edit' ELSE monitor_access.permission END AS permission
                FROM monitor
                LEFT JOIN monitor_access ON monitor_access.monitor_id = monitor.id AND monitor_access.user_id = ?
                WHERE monitor.id = ?
            `, [ userID, userID, monitor.id ]);
            const permission = permissionRow?.permission || "view";
            result[monitor.id] = {
                ...monitor.toJSON(preloadData),
                permission,
            };
        }
        return result;
