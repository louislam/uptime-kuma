...
module.exports.silenceSocketHandler = (socket) => {
    ...
    socket.on("unsilenceMonitor", async (monitorID, message, callback) => {
        try {
            checkLogin(socket);

            if (!message || !message.trim()) {
                throw new Error("A reason is required to unsilence a monitor.");
            }
            ...
            log.info("silence", `Monitor #${monitorID} unsilenced by user ${socket.userID}: ${message}`);

            // Insert a permanent log entry in the heartbeat table
            const now = R.isoDateTimeMillis(dayjs.utc());
            const currentStatus = lastBeatCheck ? lastBeatCheck.status : 0;
            const logMsg = `Unsilenced: ${message.trim()}`;
