...
function createLMResponse(challenge, lmhash) {
    ...
    let pwBuffer = new Buffer.alloc(21).fill(0);
...
function createLMHash(password) {
    let buf = new Buffer.alloc(16); let pwBuffer = new Buffer.alloc(14); let magicKey = new Buffer.from("KGS!@#$%", "ascii");
...
function createNTLMResponse(challenge, ntlmhash) {
    let buf = new Buffer.alloc(24);
    let ntlmBuffer = new Buffer.alloc(21).fill(0);
...
function createLMv2Response(type2message, username, ntlmhash, nonce, targetName) {
    let buf = new Buffer.alloc(24); let ntlm2hash = createNTLMv2Hash(ntlmhash, username, targetName); let hmac = crypto.createHmac("md5", ntlm2hash);
    //server challenge
...
function createNTLMv2Response(type2message, username, ntlmhash, nonce, targetName) {
    let buf = new Buffer.alloc(48 + type2message.targetInfo.buffer.length); let ntlm2hash = createNTLMv2Hash(ntlmhash, username, targetName); let hmac = crypto.createHmac("md5", ntlm2hash);
