 ...
 */
console.log("Welcome to Up_Or_Down");
...
// Exit Up_Or_Down immediately if the Node.js version is banned
...
log.info("server", "Up_Or_Down Version: " + checkVersion.version);
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error("Up_Or_Down has been initialized. If you want to run setup again, please delete the database.");
                }
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new Up_Or_Down instance, need to setup
    if ((await R.knex("user").count("id as count").first()).count === 0) {
