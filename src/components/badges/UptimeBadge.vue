<template>
    <div class="my-3">
        ...
        <div v-if="visible" class="border-start ps-2">
            <LabelInput :modelValue="values.interval" placeholder="24" label="Interval" @update:modelValue="set('interval', $event)" />
            <div class="my-2">
                ...
                <div :class="!labelsVisible && 'collapse'">
                    <div v-for="[ propName, display, placeholder = '' ] in labelFields" :key="display">
                        <LabelInput :modelValue="values[propName]" :placeholder="placeholder" :label="display" @update:modelValue="set(propName, $event)" />
                    </div>
                </div>
            </div>
            <div class="my-2">
                ...
                <div :class="!colorsVisible && 'collapse'">
                    <div v-for="[ propName, display, placeholder = '' ] in colorFields" :key="display">
                        <ColorInput :modelValue="values[propName]" :placeholder="placeholder" :label="display" @update:modelValue="set(propName, $event)" />
                    </div>
                </div>
            </div>
            <div>Badge URL</div>
