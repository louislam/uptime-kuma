...
<template>
    <div>
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select id="filter-monitor" v-model="filterMonitorID" class="form-select">
                        ...
                        <option v-for="monitor in monitorList" :key="monitor.id" :value="monitor.id">
                            {{ monitor.name }}
                        </option>
                    </select>
                </div>
        ...
        <div class="shadow-box p-0">
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        <tr v-for="record in records" :key="record.id">
                            ...
                            <td>
                                <span v-if="record.status === 'success'" class="badge bg-success">
                                    {{ $t("Success") }}
                                </span>
            ...
            <div v-if="!loading && records.length > 0" class="d-flex justify-content-center kuma_pagination p-3">
                <pagination v-model="page" :records="totalRecords" :per-page="perPage" :options="paginationConfig" />
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        monitorList() {
            return Object.values(this.$root.monitorList).sort((a, b) => a.name.localeCompare(b.name));
        },

        notificationList() {
            return [...this.$root.notificationList].sort((a, b) => a.name.localeCompare(b.name));
        },
    },
